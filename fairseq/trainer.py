...
class Trainer(object):
    ...
    def __init__(self, args, task, model, model_distil_from, criterion, dummy_batch=None, oom_batch=None):
        self.args = args
        self.task = task
        ...
        self._model = model
        self._model_distil_from = model_distil_from
        self.cuda = torch.cuda.is_available() and not args.cpu
        if args.fp16:
            self._criterion = self._criterion.half()
            self._model = self._model.half()
            self._model_distil_from = self._model_distil_from.half()
        if self.cuda:
            self._criterion = self._criterion.cuda()
            self._model = self._model.cuda()
            self._model_distil_from = self._model_distil_from.cuda()
        ...
        self._wrapped_model_distil_from = None
    ...
    def init_meters(self, args):
        ...
        self.meters["train_loss_mlm"] = AverageMeter()
        self.meters["train_loss_distil"] = AverageMeter()
        self.meters["valid_loss"] = AverageMeter()
        self.meters["valid_loss_mlm"] = AverageMeter()
        self.meters["valid_loss_distil"] = AverageMeter()
    ...
    @property
    def model_distil_from(self):
        if self._wrapped_model_distil_from is None:
            if self.args.distributed_world_size > 1 and not self.args.use_bmuf:
                self._wrapped_model_distil_from = models.DistributedFairseqModel(
                    self.args, self._model_distil_from
                )
            else:
                self._wrapped_model_distil_from = self._model_distil_from
        return self._wrapped_model_distil_from
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size, logging_output = self.task.train_step(
                        sample, self.model, self.model_distil_from, self.get_num_updates(), self.criterion, self.optimizer, ignore_grad
                    )
        ...
        try:
            ...
            if "loss_mlm" in logging_output:
                self.meters["train_loss_mlm"].update(
                    logging_output.get("loss_mlm", 0), ntokens
                )
            if "loss_distil" in logging_output:
                self.meters["train_loss_distil"].update(
                    logging_output.get("loss_distil", 0), ntokens
                )
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, self.model_distil_from, self.get_num_updates(), self.criterion
                )
            except RuntimeError as e:
                if "out of memory" in str(e):
                    self._log_oom(e)
        ...
        if "loss_mlm" in logging_output:
            self.meters["valid_loss_mlm"].update(
                logging_output.get("loss_mlm", 0), ntokens
            )

        if "loss_distil" in logging_output:
            self.meters["valid_loss_distil"].update(
                logging_output.get("loss_distil", 0), ntokens
            )
    ...
    def get_model_distil_from(self):
        """Get the (non-wrapped) model instance."""
        return self._model_distil_from
